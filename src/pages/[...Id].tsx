...
const Watch = () => {
  ...
  const trendingMovie = useSelector((state:any)=>state.trendingMovie.trendingMovies);
  if(!trendingMovie){
    redirect("/");
  }
  ...
  return (
        ...
        <div className='flex flex-col h-3/4 justify-around pl-6 selection:bg-neutral-50'>
            ...
            <div className='flex flex-row gap-4 pl-6'>
              <Link to={`/watch/video/${videoKey}`}>
                <button className='px-6 py-2 rounded-md shadow-md shadow-slate-800 text-slate-50 bg-slate-950 font-semibold  font-sans cursor-pointer'>Trailer</button>
              </Link>
    ...
    <h2 className="text-white font-semibold mt-5 pl-2 sticky top-1">More Like This</h2>
        ...
        {trendingMovie?.map((movie:any,index:number)=>{
